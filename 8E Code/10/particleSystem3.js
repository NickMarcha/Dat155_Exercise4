...
//var pointsArray = new Float32Array(maxNumParticles+24);
//var colorsArray = new Float32Array(maxNumParticles+24);
var pointsArray =  MVbuffer(maxNumParticles+24);
var subPointsArray = pointsArray.buf.subarray(0, 16*initialNumParticles+24);
var colorsArray =  MVbuffer(maxNumParticles+24);
var subColorsArray = colorsArray.buf.subarray(0, 16*initialNumParticles+24);
...
var simulation = function(){
    ...
    for ( var i = 0; i < numParticles; i++ ) {
        ...
        particleSystem[i].velocity[3] = 0.0;
    }
    ...
    gl.bindBuffer( gl.ARRAY_BUFFER, cBufferId );
    //gl.bufferSubData(gl.ARRAY_BUFFER, 0, colorsArray.buf);
    gl.bufferSubData(gl.ARRAY_BUFFER, 0, subColorsArray);

    gl.bindBuffer( gl.ARRAY_BUFFER, vBufferId );
    //gl.bufferSubData(gl.ARRAY_BUFFER, 0, pointsArray.buf);
    gl.bufferSubData(gl.ARRAY_BUFFER, 0, subPointsArray);
        ...
        for ( var ParticleK = 0; ParticleK < numParticles; ParticleK++ ) { /* repulsive force */
            if ( ParticleK != ParticleI ){
               var t =
                ...
                var d2 = dot(t, t);
                force = add(force, mult(0.01/d2, t));
                }
        }
    return ( force );
}
...
var update = function(){
    for (var i = 0; i < numParticles; i++ ) {
            particleSystem[i].position = add( particleSystem[i].position, mult(speed*dt, particleSystem[i].velocity));
            particleSystem[i].velocity = add( particleSystem[i].velocity, mult(speed*dt/ particleSystem[i].mass, forces(i)));
        }
